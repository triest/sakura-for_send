...
class PresentController extends Controller
{
    ...
    public function storepresent(Request $request)
    {
        ...
        if (Input::hasFile('file')) {
            ...
            $temp_file = base_path().'/public/presents/upload/'
            ...
            $small = base_path().'/public/presents/small/'
                .strtolower($image_new_name.'.'.$image_extension);
    ...
    public function givepresent(Request $request)
    {
        ...
        $girl = Girl::select(['id', 'user_id'])->where('id', $request->user_id)
            ->first();
        //   $target = User::select(['id'])->where('id', $girl->user_id)->first();
        $gift = Present::select(['id'])->where('id', $request->present_id)
            ->first();
        $giftAct = new GiftAct();
    ...
    {
        $giftAct = GiftAct::select(['id', 'present_id', 'who_id', 'target_id'])
            ->where('id', 1)->first();
        broadcast(new eventPreasent($giftAct));
    }
